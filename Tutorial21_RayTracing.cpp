...
void Tutorial21_RayTracing::UpdateTLAS()
{
    ...
    const int  CurrentInstanceCount = NumInstancesBase + m_NumActiveSpheres;
    ...
    if (!m_pTLAS || CurrentInstanceCount != PrevInstanceCount)
    {
        ...
        // También liberamos el SBT para que se reconstruya después de la TLAS
        m_pSBT.Release();
    }
    ...
    if (m_SphereInstanceNames.empty())
    {
        m_SphereInstanceNames.resize(NumSpheres);
        for (int i = 0; i < NumSpheres; ++i)
        {
            m_SphereInstanceNames[i] = "Sphere Number " + std::to_string(i + 4) + " Instance";
        }
    }
    ...
    for (int i = 0; i < m_NumActiveSpheres; ++i)
    {
        ...
        Instances[i + NumInstancesBase].InstanceName = m_SphereInstanceNames[i].c_str();
    ...
    // Reconstruir SBT si es necesario
    if (!m_pSBT)
    {
        CreateSBT();
    }
...
void Tutorial21_RayTracing::CreateSBT()
{
    ...
    for (int i = 0; i < m_NumActiveSpheres; ++i)
    {
        // Usar los nombres almacenados para consistencia
        const char* name = m_SphereInstanceNames[i].c_str();
        ...
        // También vincula el shader de sombras para esta esfera
        m_pSBT->BindHitGroupForInstance(m_pTLAS, name, SHADOW_RAY_INDEX, "SphereShadowHit");
    }
...
void Tutorial21_RayTracing::UpdateUI()
{
    ...
    // Guardar valor previo para detectar cambios
    int prevNumActiveSpheres = m_NumActiveSpheres;
    ...
    if (ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        ImGui::SliderInt("Sphere count", &m_NumActiveSpheres, 0, NumSpheres);
    ...
    // Si cambió el número de esferas activas, forzar reconstrucción de TLAS
    if (prevNumActiveSpheres != m_NumActiveSpheres)
    {
        if (m_pTLAS)
        {
            m_pTLAS.Release(); // Esto forzará la reconstrucción completa en el siguiente frame
        }
    }
}
} // namespace Diligent